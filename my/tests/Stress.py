...
useFakePLC = False
gmRange    = 20
pmRange    = int(gmRange * 1.2)

for arg in sys.argv[1:]:
    if arg == "-v":
        debugLevel = 1
    elif arg[:2] == "-d":
        debugLevel = int(arg[2:])
    elif arg == "-fake":
        useFakePLC = True
    elif arg[:3] == "-gm":
        gmRange = int(arg[3:])
...
def serial_init():
    ...
    if useFakePLC:
        ...
        # check to see if either exists, uses whichever one is there
    ...
    def _readline( self ):
        ...
        for i in range(0,10):
            try:
                ...
                self._printX("RespRetried")

            except PlcTimeout:
                self._printX("PLC timeout")
                return None
    ...
    def run( self ):
        while True:
            ...
            if self.tag == "G:":
                ...
                self._printX("Warning: unknown response (" + self.tag + ") (" + self.cmd + ")")
...
class ThreadGM( threading.Thread ):
    ...
    def run( self ):
        ...
        for i in range(1, gmRange):
            ...
            if self.resp is None:
                self._printX("WARNING: no response from GM")
            else:
...
class ThreadPM( threading.Thread ):
    ...
    def run( self ):
        ...
        for i in range(1, pmRange):
            ...
            if self.resp is None:
                self._printX("WARNING: no response from PM")
            else:
                self._printX("rsp = " + self.resp)
