...
# TODO catch keyboard press and ask user to move lens or mirrors to move different possition
# before stoppintg the test
# prompt asking to stop the actuators in certain possition?
...
class StressTest:
	def __init__(self):
		...
		self.all_header = \
			"N," + "Time" + "," + self.lens_delta_header + self.mirror_delta_header + "," + "B4 temp.,\n"
		lens_list_hs1 = [
	...
	def run_test(self):
		...
		self.move_lenses()
		self.move_mirrors()
		while not self.stop and len(self.adb_device_id) > 1:
			self.read_B4_tepmerature()
			time.sleep(0.2)
			...
			# if len(self.adb_device_id) < 1:
			# 	exit("Device is disconnected")
			cmd = ""
...
if not dut.stop:
	...
	if dut.adb_device_id:
		raw_input("Press press enter if you want to interupt\n")
		usr_input = raw_input("Type y to confirm\n")
		while usr_input.lower() != "y":
			usr_input = raw_input("No changes, Press y if you want to interupt\n")
			continue
