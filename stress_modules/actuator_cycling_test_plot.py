#################################################
# Ligh L16 optical actuator cycling test        #
# current version V0.101                         #
# Light.co                                      #
# MAMO SARGSYAN                                 #
# V 0.1                                         #
# first version                                 #
	...
	def plot_vcm(self):
		if len(self.test_data) > 5 and self.mod_data_rng_pos[0] != 0:
			...
			for cycles in sorted(self.test_data.keys()):
				temp_data = (self.test_data[cycles]).split(",")[self.mod_data_rng_pos[3]]
				if "--" not in temp_data:  # Eliminating non integer data in plot creation
