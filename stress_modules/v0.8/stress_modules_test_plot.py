...
class StressTest:
	...
	def run_test(self):
		...
		self.raw_log_file_ref.write("N cycles,,mirror data,,,,lens data \n")
...
class GUI:
	def __init__(self, master):
		...
		func_list = (
			self.connect, self.stop, self.show_mirror_graph, self.show_lens_graph,
			self.run_test, self.open_logs)

		butt_name_list = ["Connect", "Stop", "Mirror graph", "Lens graph", "Run Test", "Open logs"]
	...
	def update_ui(self):
		self.pop_up_lbl.after(700, self.update_ui)
		self.live_status_lbl.config(text="{} cycles are done".format(self.test_module.cycle_count))
		if self.test_module.is_runing:
			self.run_status.config(text="Running on: ..{}".format(self.test_module.adb_device_id[-3:]))
		else:
			self.run_status.config(text="Stopped")
