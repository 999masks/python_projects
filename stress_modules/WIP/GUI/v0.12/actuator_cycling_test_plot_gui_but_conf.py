...
class GUI:
	def __init__(self, master):
		...
		self.pop_up_lbl = Label(self.mainframe, text="Choose the unit ")
		...
		for i_1 in range(13):
			self.mainframe.rowconfigure(i_1, weight=1, minsize=45)
		...
		mod_a_check_but = Checkbutton(self.mainframe, text="VCM", var=self.VCM_select_var)
		mod_a_check_but.grid(row=3, column=1, sticky=W)

		mod_l_check_but = Checkbutton(self.mainframe, text="Lens", var=self.L_select_var)
		mod_l_check_but.grid(row=5, column=1, sticky=W)

		mod_m_check_but = Checkbutton(self.mainframe, text="Mirror", var=self.M_select_var)
		...
		for i_column in range(2):
			for i_row in range(1, 6):
				if i_column == 1 and 2 < i_row < 6:  # Avoid put button on 2 column row 1,2
					continue
				bt = Button(self.mainframe, text=butt_name_list[i_3], command=func_list[i_3])
				bt.configure(height=2, width=12)
				# disable corresponding buttons if uncheked
