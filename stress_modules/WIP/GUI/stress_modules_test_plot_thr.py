...
class CycleTest:
	def __init__(self):
		self.current_version = "0.101"
		print "Module test program, version 0.10"
	...
	def read_battery_capcaity(self):
		...
		level = self.ThreadingExample(cmd)
		print "level", level
	...
	def do_charge(self, need_charge):
		...
		if need_charge:
			...
			if self.charging_status != str(1):
				print "There is problem charging. Trying to reenable"
				time.sleep(1)
				self.send_command(cmd)
			...
			if self.charging_status != str(0):
				print "Unable to disable charging, will try once"
				time.sleep(1)
				self.send_command(cmd)
		...
		if len(self.test_data) > 5 and self.mod_data_rng_pos[0] != 0:
			...
			for cycles in sorted(self.test_data.keys()):
				if cycles % sampling_rate == 0:
					...
					b4__temp.append((self.test_data[cycles]).split(",")[self.mod_data_rng_pos[3]])
	...
	def plot_lenses(self):
		if len(self.test_data) > 5 and self.mod_data_rng_pos[1] != 0:
			...
			print "No lense data is availabe"
		...
		while not self.stop:
			...
			if str(self.read_battery_capcaity()).isdigit() and self.read_battery_capcaity() < 10:
				if not self.file_ref.closed:
					self.file_ref.close()
				print "Test stoped due to critical battery, files are saved"
				while self.read_battery_capcaity() < 95 and not self.stop:
					self.is_paused = True
					self.do_charge(True)
					time.sleep(5)
...
class GUI:
	def __init__(self, master):
		...
		self.messages = [
			"Stopped", "Selected-", "Please connect!!!", "Stop cycle first!", "No device", "Running", "Charging"]
		...
		self.run_status.grid(row=2, column=3, sticky=W)
