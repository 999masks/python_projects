...
class StressTest:
	...
	def record_data_header(self):
		self.file_ref = open((self.adb_device_id + "_actuator_debug.csv"), "a+")
		self.file_ref_1 = open((self.adb_device_id + "_actuator_clean.csv"), "a+")
		...
		self.file_ref_1.write(self.all_header)
		self.file_ref_1.close()
		self.file_ref.close()
		...
		if len(data) > 0:
			...
			for i in self.all_header.split(",")[:27]:
				if i != '' and not "HS" in i and len(i) > 1 and not "N" in i:
					...
					temp_row_1 = temp_row_1 + str((data[i]).split("-")[0]) + ","
				else:
					temp_row = temp_row + ","
					temp_row_1 = temp_row_1 + ","
			for i_1 in self.all_header.split(",")[27:]:
				if i_1 != '' and not "HS" in i_1 and len(i_1) > 1 and not "N" in i:
					temp_row = temp_row + str((data[i_1])) + ","
					temp_row_1 = temp_row_1 + str((data[i_1]).split("-")[0]) + ","
				else:
					temp_row = temp_row + ","
					temp_row_1 = temp_row_1 + ","
			temp_row = str(self.cycle_count) + temp_row + "\n"
			temp_row_1 = str(self.cycle_count) + temp_row_1 + "\n"
			self.file_ref = open((self.adb_device_id + "_actuator_debug.csv"), "a+")
			self.file_ref_1 = open((self.adb_device_id + "_actuator_clean.csv"), "a+")
			self.file_ref.write(temp_row)
			self.file_ref_1.write(temp_row_1)

	def run_test(self):
		self.record_data_header()
		self.raw_log_file_ref = open("raw_data.csv", "a+")
		self.raw_log_file_ref.write(("cycle N, lens_data, mir_data" + "\n"))
		...
		while not self.stop:
			...
			time.sleep(0.2)
			raw_len_val = self.move_lenses()
			data = self.calculate_possitions(self.all_lens_data, 2)
			raw_mirr_val = self.move_mirrors()
			self.calculate_possitions(self.all_mirr_data, 1)
			with open("raw_data.csv", "a+") as self.raw_log_file_ref:
				self.raw_log_file_ref.write(str(self.cycle_count) + "," + str(raw_len_val) + "," +
			                            str(raw_mirr_val) + "\n")
			time.sleep(0.5)
